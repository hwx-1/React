(function (global, factory) {
  ...
  lp$1.parseTopLevel = function() {
    ...
    while (this.tok.type !== acorn.tokTypes.eof) {
      const res = this.parseStatement();
      if(res.type.startsWith('ExportType')||res.type.startsWith('ImportType')){
        continue;
      }
      //lz 忽略大括号里面带 type 的类型导入如：import {type A, type B} from 'xxx'
      //不考虑如 import {type AType, BValue, CValue} from 'xxx' 的情况，因为 sdk 会把这条导入改写为下面2条
      //import {type AType} from 'xxx' 和 import {BValue, CValue } from 'xxx'
      if (res.type === "ImportDeclaration" &&
        res.specifiers.length > 1 &&
        res.specifiers[0].type === "ImportSpecifier" &&
        res.specifiers[0].imported.name === "type") {
        continue;
      }
      if (res.type === "ExportNamedDeclaration" &&
        res.specifiers.length > 1 &&
        res.specifiers[0].type === "ExportSpecifier" &&
        res.specifiers[0].exported.name === "type") {
        continue;
      }
      node.body.push(res);
    }
  ...
  lp$1.parseExport = function() {
    var node = this.startNode();
    this.next();
    const useType = this.eatContextual("type");
    if (this.eat(acorn.tokTypes.star)) {
      ...
      this.semicolon();
      return this.finishNode(node, useType ? "ExportTypeAllDeclaration" : "ExportAllDeclaration")
    }
    if (this.eat(acorn.tokTypes._default)) {
      ...
      return this.finishNode(node, useType ? "ExportTypeDefaultDeclaration" : "ExportDefaultDeclaration")
    }
    ...
    return this.finishNode(node, useType ? "ExportTypeNamedDeclaration" : "ExportNamedDeclaration")
  };
  ...
  lp$1.parseImport = function() {
    ...
    if (this.tok.type === acorn.tokTypes.string) {
      ...
      if (this.tok.type === acorn.tokTypes.name && this.tok.value !== "from") {
        ...
        this.eat(acorn.tokTypes.comma);
        //lz
        if (elt.local.name === 'lazy' && !this.isContextual('from')) {
          elt = undefined;
        }
      }
    ...
    this.semicolon();
    return this.finishNode(node, useType? "ImportTypeDeclaration": "ImportDeclaration")
  };
  ...
  lp.parseExprAtom = function() {
    ...
    default:
      //lz
      const skipNext = this.tok.type.label === ":" ? true : false;
      const dummyIdent = this.dummyIdent();
      skipNext && this.next();
      return dummyIdent;
    }
  };
