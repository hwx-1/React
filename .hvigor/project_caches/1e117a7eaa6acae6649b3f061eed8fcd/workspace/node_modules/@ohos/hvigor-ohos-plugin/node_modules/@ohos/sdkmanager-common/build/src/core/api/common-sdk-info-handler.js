...
class CommonSdkInfoHandler {
    ...
    doGetOrDownload(pathAndApiVersions, localSdks, downConsumer) {
        return __awaiter(this, void 0, void 0, function* () {
            const existRequiredSdks = this._filterRequiredSdks(localSdks, pathAndApiVersions);
    ...
    _throwNotFoundException(notFoundComponents) {
        ...
        for (const pathAndApiVersion of notFoundComponents) {
            ...
            if ((0, utils_1.isValidApiVersion)(pathAndApiVersion.getFullApiVersion())) {
                msg += `${os_1.default.EOL}\t\t${path}:${(_a = pathAndApiVersion.getFullApiVersion()) === null || _a === void 0 ? void 0 : _a.getValue()}`;
            }
            else {
...
\t\t2.Alternatively, modify the compileSdkVersion settings in the project-level build-profile.json5 file.`));
    ...
    mapComponents(components) {
        const map = components.map(value => {
            const tuple = [
                new path_and_apiversion_1.PathAndApiVersion(value.getPath(), value.getFullApiVersion()),
                value,
            ];
            return tuple;
        });
