...
const getRealStackTrace = async (stackTraceParams) => {
    const testType = stackTraceParams.isLocalTest ? unitTest : ohosTest;
    ...
    let sourceMapInnerJson = runSourceMap[filePathFromStackTrace];
    if (!sourceMapInnerJson) {
        return (0, StackTraceUtil_1.getString)(stackTrace);
    }
    let realFilePath = sourceMapInnerJson.sources[0];
    if (realFilePath) {
        stackTrace.filePath = realFilePath;
    }
    if (filePathFromStackTrace.endsWith(JS)) {
        return (0, StackTraceUtil_1.getString)(stackTrace);
    }
    return parseRowColumn(sourceMapInnerJson, stackTrace);
};
